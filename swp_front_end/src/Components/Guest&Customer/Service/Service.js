    ...
    try {
      ...
      if (response.ok) {
        ...
        const activeServices = data.filter(
          (service) => service.serviceIsActive === 1
        );
  ...
  const getServiceContent = () => {
      ...
      return (
        <div className="content-text">
          <p style={{ fontSize: "1.5rem" }}>
            Welcome to King's Teeth Dental Clinic, where we blend
            state-of-the-art dental technology, personalized care, and a
            relaxing atmosphere to deliver top-tier oral health services. Our
            dedicated team of experienced professionals is committed to
            providing tailored solutions for all your dental needs, whether it's
            a routine check-up, cosmetic treatment, or specialized procedure. At
            King's Teeth, we prioritize your comfort and well-being, ensuring
            every smile receives the royal treatment it deserves.
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="content-text">
        ...
        <button
          className="button"
          onClick={() => HandleServiceBook(selectedService.id)}
        >
          Book this service
        </button>
      </div>
  ...
  return (
    <div className="service-container">
      ...
      <div className="service-main">
        ...
        <div className="service-content">{getServiceContent()}</div>
      </div>
      <Footer />
    </div>
  );
